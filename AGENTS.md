...
## Ритуал очереди владельца (DSH-Bridge)

Владелец может писать из Telegram-бота `dsh-bridge` (код: `C:\Projects\dsh-bridge`), когда ПК
вне доступа. Очередь — файлы в `C:\Projects\dsh-bridge\`:
- **В начале каждой сессии и периодически**: прочитать `inbox.txt` (записи `[время] текст`),
  обработать каждую (это задачи от владельца), ответы дописать в `outbox.json`
  (список `{"text": "..."}`) — бот доставит их владельцу автоматически.
- Обработанные записи в inbox не удалять, а помечать строкой `[done]` в начале записи —
  так видно, что осталось без ответа.
