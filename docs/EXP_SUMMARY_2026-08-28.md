...
(последнюю можно мержить одну — она вбирает все предыдущие).
...
| `exp/miniapp-prep` | **ПОЛНЫЙ скелет фазы 2** (содержит все ветки ниже): FastAPI `GET /menu`, `POST /orders`, `GET /orders/{id}`; initData-авторизация (HMAC-SHA256 от `BOT_TOKEN`, заголовок `X-Telegram-Init-Data`); каркас `POST /webhooks/kaspi` (идемпотентность: первое событие 200, дубль 409); **без деплоя** — нужен домен+HTTPS | **103 зелёных** (8 PG, 10 api) | 13 |
